...
import { isStorageAvailable, listStorageKeys, readStorageItem, writeStorageItem } from '../lib/storage';
...
const normalizeScopeId = (scopeId: string) => scopeId.trim() || FALLBACK_STORAGE_SCOPE;
const getBillingStorageKey = (scopeId: string) => `${BILLING_STORAGE_KEY_PREFIX}${normalizeScopeId(scopeId)}`;
...
const getInitialStorageScopeId = (): string => {
  const globalUserState = readStorageItem(LEGACY_GLOBAL_USER_STATE_KEY);
  if (!globalUserState) {
    return FALLBACK_STORAGE_SCOPE;
  }

  try {
    const parsed = JSON.parse(globalUserState) as { internalUserId?: string };
    return normalizeScopeId(parsed.internalUserId ?? FALLBACK_STORAGE_SCOPE);
  } catch {
    return FALLBACK_STORAGE_SCOPE;
  }
};

const getInitialBillingState = (scopeId: string, allowLegacyImport: boolean): BillingStateSnapshot => {
  const savedStateJSON = readStorageItem(getBillingStorageKey(scopeId));
  if (!savedStateJSON) {
...
export const BillingProvider = ({ children }: { children: ReactNode }) => {
  const initialStorageScopeId = useMemo(() => getInitialStorageScopeId(), []);
  const initialState = useMemo(() => getInitialBillingState(initialStorageScopeId, true), [initialStorageScopeId]);
  const [storageScopeId, setStorageScopeId] = useState(initialStorageScopeId);
  ...
  useEffect(() => {
    ...
    const scopedState = getInitialBillingState(storageScopeId, false);
    setCartsByArtist(scopedState.cartsByArtist);
    setHasCard(scopedState.hasCard);
    setOrders(scopedState.orders);
    setPaymentHistory(scopedState.paymentHistory);
    setPaymentMethod(scopedState.paymentMethod);
    setPurchasedExperiences(scopedState.purchasedExperiences);
    setPurchasedTickets(scopedState.purchasedTickets);
  }, [storageScopeId]);

  useEffect(() => {
    writeStorageItem(
      getBillingStorageKey(storageScopeId),
      JSON.stringify({
        cartsByArtist,
        hasCard,
        orders,
  ...
  }, [cartsByArtist, hasCard, orders, paymentHistory, paymentMethod, purchasedExperiences, purchasedTickets, storageScopeId]);
  ...
  const setStorageScope = useCallback((scopeId: string) => {
    const normalizedScope = normalizeScopeId(scopeId);
    setStorageScopeId((previousScope) => (previousScope === normalizedScope ? previousScope : normalizedScope));
  }, []);
