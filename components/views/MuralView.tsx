...
const FanAreaSubSection: React.FC<{title: string, onBack: () => void; onPostClick: () => void; children: React.ReactNode}> = ({ title, onBack, onPostClick, children }) => (
  <div className="p-4 animate-fade-in pb-24">
    <header className="flex items-center justify-between mb-6">
      ...
      <button 
        onClick={onPostClick}
        className="bg-rose-500 text-white font-black py-2.5 px-5 rounded-2xl hover:bg-rose-600 transition-all text-xs flex items-center space-x-2 shadow-lg shadow-rose-500/20"
      >
...
const MuralView: React.FC<MuralViewProps> = ({ onBack, posts, likedPostIds, onLikePost, onViewImage, onAddPost }) => {
  ...
  return (
    <>
        <FanAreaSubSection title="Mural de Fãs" onBack={onBack} onPostClick={() => setIsPostingModalVisible(true)}>
